...
class MediaButtonListener:
    ...
    def _on_button(self, sender, args):
        ...
        if cb is None:
            return
        now = time.monotonic()
        # Debounce per *callback*, not per button: a headset that toggles between
        # PLAY and PAUSE codes on successive presses still lands on the same
        # callback, and duplicates must be judged against that shared timeline.
        key = id(cb)
        if now - self._last.get(key, 0) < self._debounce_s:
            return
        self._last[key] = now
    ...
    def duck(self, seconds: float = 1.0):
        """Briefly pause the silent keepalive, then resume. Called on every
        button press so the press the headset just sent as 'pause' is visibly
        honoured — a state-tracking dongle (Yealink) swallows the next press if
        the host's audio just keeps playing through one. Repeated presses
        (multi-click gestures) restart the resume timer."""
        if not self._keepalive or self._player is None:
            return
        with self._duck_lock:
            try:
                self._player.pause()
            except Exception:  # noqa: BLE001
                log.exception("keepalive pause failed")
                return
            if self._duck_timer is not None:
                self._duck_timer.cancel()
            self._duck_timer = threading.Timer(seconds, self._duck_resume)
            self._duck_timer.daemon = True
            self._duck_timer.start()

    def _duck_resume(self):
        with self._duck_lock:
            self._duck_timer = None
            try:
                self._player.play()
            except Exception:  # noqa: BLE001
                log.exception("keepalive resume failed")
